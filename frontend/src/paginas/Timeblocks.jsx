...
function Timeblocks() {
  ...
  const handleToDeleteTimeblock = async (id) => {
    try {
      ...
      setTimeblocks(
        (prev) => prev.filter((timeblock) => timeblock.id !== id), // actualizamos el estado con los demas timeblocks excepto el id seleccionado
      );
    } catch (error) {
      setError("Error al eliminar el horario");
    }
  ...
  const handleEditTimeBlock = async (id) => {
    try {
      ...
      setTimeblocks((prev) => prev.map((tb) => (tb.id === id ? data : tb)));
  ...
  return (
    <section className="tb-page">
      <div className="tb-header">
        ...
        <button className="tb-button" onClick={() => setShowForm(!showForm)}>
          + Nuevo horario
        </button>
      </div>
      ...
      <section className="tb-stats">
        ...
        <article className="tb-stat-card">
          ...
          <strong>
            {timeblocks.filter((tb) => tb.appointments?.length > 0).length}
          </strong>
        </article>
      </section>
                  ...
                  timeblocks.map((timeblock) => {
                    ...
                    return (
                      <div key={timeblock.id} className="tb-time-card">
                        ...
                        <p
                          className={`tb-reserved-text ${!reservado ? "empty" : ""}`}
                        >
                        ...
                        <div className="tb-time-actions">
                          ...
                          <button
                            ...
                            disabled={reservado}
                            onClick={() =>
                              handleToDeleteTimeblock(timeblock.id)
                            }
                          >
                            Eliminar
                          </button>
